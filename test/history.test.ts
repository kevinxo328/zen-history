...
import { TimeRangeType, KeepRecentValue, RemoveRecentValue } from '@/types/clean-settings';
...
describe('cleanHistory', () => {
  ...
  it('calculates time correctly for REMOVE_RECENT: Past 1 Hour', async () => {
    await cleanHistory({ type: TimeRangeType.REMOVE_RECENT, value: RemoveRecentValue.PAST_ONE_HOUR });
    
    // expect search to be called with correct startTime (1 hour ago)
    const oneHourAgo = new Date('2026-03-12T11:00:00.000Z').getTime();
    expect(browserMock.history.search).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('skips deleteRange if search returns no results', async () => {
    ...
    const result = await cleanHistory({ type: TimeRangeType.REMOVE_RECENT, value: RemoveRecentValue.PAST_ONE_HOUR });
